<template>
    <div class="map">
        <h3>Maptree</h3>
        <canvas id='maptree' style="width: 100%;height: 400px;"></canvas>
    </div>
</template>
...
export default {
    ...
    mounted(){
        ...
        console.log(DataSet);
        console.log(this.F2.Shape);
        this.F2.Shape.registerShape('polygon', 'labeledPolygon', {
            draw: function draw(cfg, container) {
                const points = this.parsePoints(cfg.points);
                const style = this.F2.Util.mix({
                fill: cfg.color,
                points
                }, cfg.style);
